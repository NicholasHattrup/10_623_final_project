...
class TrainConfig:
    datapath : str = field(
        metadata = {'description' : "Path of dataset to be used for training."}
    )
    outpath : str = field(
        metadata = {'description' : "Folder where output directory will be created to save results."}
        
    )
    datapath_opt: str = field(
        default = None,
        metadata = {'description' : "Optional secondary dataset to be used for training"}
    )
    n_epochs : int = field(
        default = 5
    )
...
def train(fabric, cfg: TrainConfig, out_dir : str, padding_label = -1):
    ...
    rdkit_molecules = parse_molecules(cfg.datapath_opt) if cfg.datapath_opt is not None else None


    # Generate RDKit Molecules with ETKDGv3
    if rdkit_molecules is None:
        ...
        low_quality_mol_features = {}

        # 1) build low-quality mols with a progress bar
        low_quality_mols = {
            ss: generate_molecule_from_smiles(ss)
            for ss in tqdm(list(smiles_strs), desc="Low-Quality Structures")
        }

        # 2) featurize each one (pulling in the pre-computed positions) with its own bar
        for ss, mol in tqdm(low_quality_mols.items(), desc="Featurizing", total=len(low_quality_mols)):
            positions = rdkit_molecules[ss].positions
            low_quality_mol_features[ss] = featurize_mol(mol, False, True, positions)
            
    
    # with open(os.path.join(out_dir, "low_quality_mol_features.pkl"), "wb") as f:
    #     pickle.dump(low_quality_mols, f)

    with open(os.path.join(out_dir, "low_quality_mol_features.pkl"), "wb") as f:
        pickle.dump(low_quality_mol_features, f)
    with open(os.path.join(out_dir, "low_quality_mols.pkl"), "wb") as f:
        pickle.dump(low_quality_mols, f)
